...
public class ComboBoxTest3 extends JFrame implements ItemListener {
    static final long serialVersionUID = 0L;
    
    private static String[] fontItems = GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames();
    private JComboBox fontComboBox = new JComboBox(fontItems);
    private JLabel displayLabel = new JLabel(fontItems[0]);
    
    private static String[] colors = {"Schwarz", "Rot","Gruen", "Blau", "Gelb"};
    ...
    public void itemStateChanged(ItemEvent e) {
        if (e.getSource() == fontComboBox) {
          ...
          //displayLabel.setText((String)e.getItem());
            displayLabel.setFont(new Font((String)fontComboBox.getSelectedItem(), Font.PLAIN, 20));
        }
        if(e.getSource() == colorCombo){
        		...
        		if(((String)(colorCombo.getSelectedItem())).equals("Schwarz")){
