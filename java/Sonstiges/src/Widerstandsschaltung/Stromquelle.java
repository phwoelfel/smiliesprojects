...
public class Stromquelle {
	...
	public Stromquelle(int sp){
		widerstand=0;
		spannung=sp;
		strom=0;
	}
	public Stromquelle(int sp, int xa, int ya){
		widerstand=0;
		spannung=sp;
		strom=0;
		x=xa;
	...
	public Stromquelle(int sp, int xa, int ya, int posi){
		widerstand=0;
		spannung=sp;
		strom=0;
		x=xa;
	...
	public void draw(Graphics g, int mode){
		if(pos==2){
			...
			if(mode==1){
				g.drawString(spannung +" A", x+5, y+35);
			}
		}
		else{
			g.drawOval(x, y, 20, 20);
			...
			if(mode==1){
				g.drawString(spannung +" A", x+25, y+15);
			}
		}
	}
