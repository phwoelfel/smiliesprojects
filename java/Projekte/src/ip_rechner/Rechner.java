...
public class Rechner extends JFrame implements ActionListener{
	...
	public Rechner(){
		...
		lb_cont[1] = new JLabel(""); //SNM
		lb_cont[2] = new JLabel(""); //Klassennetz
		...
		tf_ip.setText("10.180.10.18");
		tf_snm.setText("255.192.0.0");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==bt_go){
			...
			lb_cont[1].setText("SNM: " +snm);
			lb_cont[2].setText("Kl. Netz: " +cl_net);
			...
			lb_cont[10].setText("Anz H.: " +(getMN(snm)-2));
	...
	public IP getSNAdress(IP ip, IP snm) throws Exception{
		int[] sq = getSexyQuad(ip, snm);
		int mn = getMN(snm);
		int snq = (sq[0]/mn)*mn;
		int[] ipadr = new int[4];
		
		
		System.out.println("sq: " +sq[0] +", " +sq[1]);
		System.out.println("mn: " +mn);
		System.out.println("snq: " +snq);
		
		for(int i=0;i<sq[1];i++){
			ipadr[i]=ip.getAdress()[i];
		}
		ipadr[sq[1]]=snq;
		for(int i=sq[1]+1;i<4;i++){
			ipadr[i]=0;
		}
		try {
			return new IP(ipadr);
